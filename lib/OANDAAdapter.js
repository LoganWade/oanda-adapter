...
function OandaAdapter (config) {
    ...
    config.api_version = config.api_version || "v1";
...
OandaAdapter.prototype.getAccounts = function (callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/accounts" + (this.username ? "?username=" + this.username : "")
...
OandaAdapter.prototype.getAccount = function (accountId, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/accounts/" + accountId
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype.getOpenPositions = function (accountId, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/accounts/" + accountId + "/positions"
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype.getInstruments = function (accountId, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/instruments?accountId=" + accountId + "&fields=" + ["instrument", "displayName", "pip", "maxTradeUnits", "precision", "maxTrailingStop", "minTrailingStop", "marginRate", "halted"].join("%2C"),
    },
    function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
...
OandaAdapter.prototype.getPrice = function (symbol, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/prices?instruments=" + symbol
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype._streamPrices = function (accountId) {
    ...
    this.pricesRequest = httpClient.sendRequest({
            hostname: this.streamHost,
            method: "GET",
            path: "/" + config.api_version + "/prices?accountId=" + accountId + "&instruments=" + this.priceSubscriptions,
            headers: {
                Authorization: "Bearer " + this.accessToken,
                Connection: "Keep-Alive"
            },
...
OandaAdapter.prototype.getCandles = function (symbol, start, end, granularity, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/candles?" + querystring.stringify({
            instrument: symbol,
...
OandaAdapter.prototype.getOpenTrades = function (accountId, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/" + config.api_version + "/accounts/" + accountId + "/trades"
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype.createOrder = function (accountId, order, callback) {
    ...
    this._sendRESTRequest({
        method: "POST",
        path: "/" + config.api_version + "/accounts/" + accountId + "/orders",
        data: order,
        headers: {
            Authorization: "Bearer " + this.accessToken,
...
OandaAdapter.prototype.closeTrade = function (accountId, tradeId, callback) {
    ...
    this._sendRESTRequest({
        method: "DELETE",
        path: "/" + config.api_version + "/accounts/" + accountId + "/trades/" + tradeId
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
